namespace OriginalStudio.BLL.SMS
{
    ...
    public class PhoneValidFactory
    {
        ...
        private static string BuilderWhere(List<SearchParam> param, List<SqlParameter> paramList)
        {
            ...
            if ((param != null) && (param.Count > 0))
            {
                for (int i = 0; i < param.Count; i++)
                {
                    ...
                    string str = param2.ParamKey.Trim().ToLower();
                    if ((str != null) && (str == "mobile"))
                    {
        ...
        public static bool PhoneSetting(int id, bool state)
        {
            try
            {
                SqlParameter[] commandParameters = new SqlParameter[] { new SqlParameter("@ID", SqlDbType.Int, 10), new SqlParameter("@enable", SqlDbType.Bit) };
