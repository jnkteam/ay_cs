        ...
        public static int GetChannelTypeStatus(int typeId, int userId)
        {
            ...
            if (cacheModel != null)
            {
                ...
                switch (cacheModel.isOpen)
                {
                    ...
                    case OpenEnum.Close:
                        num = KuaiCard.BLL.Channel.Channel.GetChanelSysStatus(4, userId, string.Empty, typeId, ref suppid);
                        break;

                    case OpenEnum.Open:
                        num = KuaiCard.BLL.Channel.Channel.GetChanelSysStatus(8, userId, string.Empty, typeId, ref suppid);
                        break;
                }
                if (num == 1)
                {
                    num = KuaiCard.BLL.Channel.Channel.GetUserOpenStatus(userId, string.Empty, typeId, 1);
                }
            }
            return num;
        }
