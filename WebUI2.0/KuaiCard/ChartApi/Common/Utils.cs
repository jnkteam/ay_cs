...
namespace General.Web
{
    public class Utils
    {
        ...
        /// <returns></returns>
        public static System.Collections.ArrayList SplitStringToList(string str, string splitflag)
        {
            int star = 0;
            bool flag = false;
            ...
            {
                star = str.IndexOf(splitflag);
                list.Add(str.Substring(0, star));
        ...
        /// <returns></returns>
        public static string FileterStr(String Str, string Filter)
        {
            return Regex.Replace(Str.Trim(), Filter, " ", RegexOptions.IgnoreCase);
        }
        ...
        {
            return System.Web.Security.FormsAuthentication.HashPasswordForStoringInConfigFile(Str.ToLower(), "MD5").ToUpper();
        }
        ...
        {
            System.Web.UI.Page curPage = (System.Web.UI.Page)System.Web.HttpContext.Current.Handler;
            strMessage = "alert('" + strMessage + "')";
            if (!curPage.ClientScript.IsStartupScriptRegistered("showmsg"))
                curPage.ClientScript.RegisterStartupScript(curPage.GetType(), "showmsg", strMessage, true);
        }
        ...
        /// </summary>
        /// <param name="strMessage">提示信息</param>
        /// <param name="strUrl">跳转url页码</param>
        public static void ShowMessage(string strMessage, string strUrl)
        {
            strMessage = "alert('" + strMessage + "'); window.location.href='" + strUrl + "';";
            System.Web.UI.Page curPage = (System.Web.UI.Page)System.Web.HttpContext.Current.Handler;
            if (!curPage.ClientScript.IsStartupScriptRegistered("showmsg"))
                curPage.ClientScript.RegisterStartupScript(curPage.GetType(), "showmsg", strMessage, true);
        }
        ...
        /// <param name="strMessage">提示信息</param>
        /// <param name="strUrl">跳转url页码</param>
        public static void ShowMessage(System.Web.UI.Page page, string strMessage, string strUrl)
        {
            strMessage = "alert('" + strMessage + "'); window.location.href='" + strUrl + "';";
            if (!page.ClientScript.IsStartupScriptRegistered("showmsg"))
                page.ClientScript.RegisterStartupScript(page.GetType(), "showmsg", strMessage, true);
        ...
        /// <summary>
        /// 绑定表格控件。（适用于WebControls）
        /// </summary>
        /// <param name="lstCtrl"></param>
        /// <param name="DtSource"></param>
        public static void BindToRepeatControl(System.Web.UI.Control Ctrl,
                                                System.Data.DataTable DtSource)
        {
            ...
            if (Ctrl is Repeater)
            {
                ((Repeater)Ctrl).DataSource = DtSource;
                ((Repeater)Ctrl).DataBind();
            }
            else if (Ctrl is DataGrid)
            {
                ((DataGrid)Ctrl).DataSource = DtSource;
                ((DataGrid)Ctrl).DataBind();
            }
            else if (Ctrl is GridView)
            {
                ((GridView)Ctrl).DataSource = DtSource;
                ((GridView)Ctrl).DataBind();
            }
        }

        /// <summary>
        /// 绑定ListControl控件。（适用于WebControls）
        /// </summary>
        /// <param name="lstCtrl"></param>
        /// <param name="DtSource"></param>
        /// <param name="DataTextField"></param>
        /// <param name="DataValueField"></param>
        public static void BindToListControl(System.Web.UI.WebControls.ListControl lstCtrl,
                                                System.Data.DataTable DtSource,
        ...
        /// <summary>
        /// 根据字符串设置ListControl选中项。
        /// </summary>
        /// <param name="chkList"></param>
        /// <param name="strValues">若涉及多个值，用“|”分割的字符串。</param>
        public static void SetListControlSelectedItem(System.Web.UI.WebControls.ListControl lstCtrl,
                                                        string strValues)
        {
            //数据源为空，不设定，退出。
            if (lstCtrl.DataSource == null) return;
            ...
            if (strValues.IndexOf("|") > -1)
            {
                ...
                {
                    string S = lstCtrl.Items[I].Value.ToString();
                    foreach (string T in Strs)
                    {
                        if (T == S)
                            lstCtrl.Items[I].Selected = true;
                    }
                }
            }
            else
            {
                lstCtrl.SelectedIndex = lstCtrl.Items.IndexOf(lstCtrl.Items.FindByValue(strValues));
            }
        }

        /// <summary>
        /// 获取ListControl控件的选择项组合。形如：“12|21|36|15”
        /// </summary>
        /// <param name="lstCtrl"></param>
        /// <returns></returns>
        public static string GetListControlSelectedItem(System.Web.UI.WebControls.ListControl lstCtrl)
        {
            StringBuilder S = new StringBuilder();
            for (int I = 0; I < lstCtrl.Items.Count; I++)
            {
                if (lstCtrl.Items[I].Selected == true)
                {
                    S.Append(lstCtrl.Items[I].Value.ToString());
                    S.Append("|");
                }
            }
            if (S.Length == 0)
        ...
        /// <summary>
        /// 获取URL中参数值（如果不存在该参数，返回空值）。
        /// </summary>
        /// <param name="Parm">参数</param>
        /// <param name="Default_Value">参数</param>
        /// <returns>参数值</returns>
        public static string GetParmValue(string Parm, string Default_Value = "")
        {
            if (System.Web.HttpContext.Current.Request.QueryString[Parm] != null)
            {
                try
                {
                    return Convert.ToString(System.Web.HttpContext.Current.Request.QueryString[Parm]);
                }
                catch
                {
                    return "";
