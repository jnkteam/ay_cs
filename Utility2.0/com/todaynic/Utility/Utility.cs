    ...
    public class Utility
    {
        ...
        public static string convertToPinYin(string chrstr)
        {
            ...
            for (int i = 0; i < chArray.Length; i++)
            {
                ...
                {
                    int num2 = ((bytes[0] * 0x100) + bytes[1]) - 0x10000;
                    if ((num2 > 0) && (num2 < 160))
                    {
        ...
        public static string CutString(string str, int length)
        {
            ...
            if (str != "")
            {
                str2 = (str.Length <= length) ? str : (str.Substring(0, length) + "...");
            }
            return str2;
        }
        ...
        {
            byte[] buffer = new MD5CryptoServiceProvider().ComputeHash(Encoding.Default.GetBytes(data));
            StringBuilder builder = new StringBuilder();
        ...
        {
            byte[] buffer = new byte[2];
            foreach (char ch in chrstr.ToCharArray())
            {
                if (Encoding.Default.GetBytes(ch.ToString()).Length == 2)
                {
                    return true;
                }
            }
        ...
        {
            string str = !(ButtonToTie is LinkButton) ? (!(ButtonToTie is ImageButton) ? ("if ((event.which && event.which == 13) || (event.keyCode && event.keyCode == 13)) {document.forms[0].elements['" + ButtonToTie.UniqueID.Replace(":", "_") + "'].click();return false;} else return true; ") : ("if ((event.which && event.which == 13) || (event.keyCode && event.keyCode == 13)) {" + page.ClientScript.GetPostBackEventReference(ButtonToTie, "").Replace(":", "$") + ";return false;} else return true;")) : ("if ((event.which && event.which == 13) || (event.keyCode && event.keyCode == 13)) {" + page.ClientScript.GetPostBackEventReference(ButtonToTie, "").Replace(":", "$") + ";return false;} else return true;");
