    ...
    public class OrderBankNotify
    {
        ...
        public void NotifyCheckStatus(object stateInfo)
        {
            try
            {
                ...
                string str2 = SystemApiHelper.GetSuccessFlag(notify.orderInfo.version);
        ...
        public string SynchronousNotify(string orderId)
        {
            ...
            OrderBank bank = new OrderBank();
            OrderBankInfo model = bank.GetOrderbankModel(orderId);
            if (model == null)
            {
                return string.Empty;
            }
            string str = SystemApiHelper.GetSuccessFlag(model.version);
            ...
            try
            {
                if (PageValidate.IsUrl(callBackUrl))
                {
                    ...
                    //OriginalStudio.Lib.Logging.LogHelper.Write("KuaiCard.BLL.Order.SynchronousNotify callBackUrl地址:" + callBackUrl);
                    //OriginalStudio.Lib.Logging.LogHelper.Write("KuaiCard.BLL.Order.SynchronousNotify返回:" + str3 + Environment.NewLine + str + Environment.NewLine + model.notifystat.ToString());

                    if ((str3.StartsWith(str) || str3.ToLower().StartsWith(str) || str3.ToLower().EndsWith("pstate=0")) && (model.notifystat != 2))
                    {
                        //OriginalStudio.Lib.Logging.LogHelper.Write("通知:" + model.orderid);
