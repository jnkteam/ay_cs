namespace OriginalStudio.BLL.Withdraw
{
    ...
    {
        private readonly OriginalStudio.DAL.Withdraw.settledAgent dal = new OriginalStudio.DAL.Withdraw.settledAgent();
        ...
        public List<OriginalStudio.Model.Withdraw.settledAgent> DataTableToList(DataTable dt)
        {
            List<OriginalStudio.Model.Withdraw.settledAgent> list = new List<OriginalStudio.Model.Withdraw.settledAgent>();
            int count = dt.Rows.Count;
            if (count > 0)
            {
                for (int i = 0; i < count; i++)
                {
                    OriginalStudio.Model.Withdraw.settledAgent item = this.dal.DataRowToModel(dt.Rows[i]);
                    if (item != null)
                    {
                        list.Add(item);
                    }
        ...
        {
            OriginalStudio.Model.Withdraw.settledAgent model = this.GetModel(trade_no);
            if (model != null)
            {
        ...
        public List<OriginalStudio.Model.Withdraw.settledAgent> GetModelList(string strWhere)
        {
            try
            {
