...
namespace OriginalStudio.Lib.Utils
{
	public class Common
	{
		...
		private static string TemplateCookieName = string.Format("dnttemplateid_{0}_{1}_{2}", Common.AssemblyFileVersion.FileMajorPart, Common.AssemblyFileVersion.FileMinorPart, Common.AssemblyFileVersion.FileBuildPart);
		...
		{
			Match match = Common.RegexBr.Match(str);
			while (match.Success)
			{
		...
		{
			return string.Format("{0}.{1}.{2}", Common.AssemblyFileVersion.FileMajorPart, Common.AssemblyFileVersion.FileMinorPart, Common.AssemblyFileVersion.FileBuildPart);
		}
		...
		{
			return Common.GetPageNumbers(curPage, countPage, url, extendPage, pagetag, null);
		}
		...
		{
			string[] strArray = Common.SplitString(ip, ".");
			int index = 0;
			bool result;
			while (index < iparray.Length)
			{
				string[] strArray2 = Common.SplitString(iparray[index], ".");
				int num = 0;
		...
		public static bool IsCompriseStr(string str, string stringarray, string strsplit)
		{
			...
			{
				str = str.ToLower();
				string[] array = Common.SplitString(stringarray.ToLower(), strsplit);
		...
		public static bool IsRuleTip(Hashtable NewHash, string ruletype, out string key)
		{
			...
			foreach (DictionaryEntry entry in NewHash)
			{
				try
				{
					string[] strArray = Common.SplitString(entry.Value.ToString(), "\r\n");
								...
								if (!(text == "email"))
								{
									if (!(text == "ip"))
									{
										if (text == "timesect")
										{
											...
											if (!Common.IsTime(strArray2[1].ToString()) || !Common.IsTime(strArray2[0].ToString()))
											{
												throw new Exception();
											}
										}
									}
									else if (!Common.IsIPSect(str.ToString()))
									{
										throw new Exception();
									}
								}
								else if (!Common.IsValidDoEmail(str.ToString()))
								{
									throw new Exception();
								}
							}
		...
		{
			return Common.RestoreFile(backupFileName, targetFileName, null);
		}
