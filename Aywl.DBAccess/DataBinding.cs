namespace OriginalStudio.DBAccess
{
    ...
    public sealed class DataBinding
    {
        public static string BuildInsertCommandText(string tableName, params string[] fields)
        {
            ...
            string str4 = str.Remove(str.Length - 2);
            string str5 = str2.Remove(str2.Length - 2);
            return string.Format("INSERT INTO [{0}] ( {1} ) VALUES ( {2} )", tableName, str4, str5);
        }
        ...
        public static List<SqlParameter> BuildParameter(string commandText, object dataObj, bool checkParameter)
        {
            ...
            foreach (Match match in matchs)
            {
                ...
                {
                    object obj2 = property.GetValue(dataObj, null);
                    SqlParameter item = new SqlParameter("@" + name, obj2);
        ...
        public static string BuildUpdateCommandText(string tableName, string condition, params string[] fields)
        {
            ...
            string str3 = str.Remove(str.Length - 2);
            condition = condition.Trim();
            if (!(string.IsNullOrEmpty(condition) || condition.StartsWith("WHERE", true, null)))
            {
                condition = "WHERE " + condition;
            }
            return string.Format("UPDATE [{0}] SET {1} {2}", tableName, str3, condition);
        }
