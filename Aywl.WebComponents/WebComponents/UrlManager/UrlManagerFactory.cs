    ...
    public class UrlManagerFactory : IHttpHandlerFactory
    {
        public IHttpHandler GetHandler(HttpContext context, string requestType, string url, string pathTranslated)
        {
            ...
            if (config != null)
            {
                ...
                return new OriginalStudio.WebComponents.UrlManager.StaticFileHandler(context, config, simplePath, realPath, filePath, pathInfo, queryString, str6);
            }
            if (Path.GetExtension(context.Request.Path) == ".aspx")
            {
                return new PageHandler();
            }
            return new OriginalStudio.WebComponents.UrlManager.StaticFileHandler();
        }
