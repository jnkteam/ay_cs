namespace KuaiCard.BLL.Withdraw
{
    ...
    {
        private readonly KuaiCard.DAL.Withdraw.settledAgentNotify dal = new KuaiCard.DAL.Withdraw.settledAgentNotify();
        ...
        public List<KuaiCard.Model.Withdraw.settledAgentNotify> DataTableToList(DataTable dt)
        {
            List<KuaiCard.Model.Withdraw.settledAgentNotify> list = new List<KuaiCard.Model.Withdraw.settledAgentNotify>();
            int count = dt.Rows.Count;
            if (count > 0)
            {
                for (int i = 0; i < count; i++)
                {
                    KuaiCard.Model.Withdraw.settledAgentNotify item = this.dal.DataRowToModel(dt.Rows[i]);
                    if (item != null)
                    {
                        list.Add(item);
                    }
        ...
        public void DoNotify(KuaiCard.Model.Withdraw.settledAgent model)
        {
            if (((model != null) && PageValidate.IsUrl(model.return_url)) && !model.is_cancel)
            {
                ...
                new KuaiCard.BLL.Withdraw.settledAgentNotify().Add(notify);
            }
        }
        ...
        public List<KuaiCard.Model.Withdraw.settledAgentNotify> GetModelList(string strWhere)
        {
