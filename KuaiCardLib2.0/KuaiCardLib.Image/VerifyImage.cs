	...
	public class VerifyImage
	{
		...
		public VerifyImageInfo GenerateImage(string code, int width, int height, Color bgcolor, int textcolor)
		{
			...
			for (int index = 0; index < code.Length; index++)
			{
				...
				graphics2.DrawString(code[index].ToString(), VerifyImage.fonts[VerifyImage.Next(VerifyImage.fonts.Length - 1)], solidBrush, new PointF(0f, 0f));
				graphics2.ResetTransform();
				graphics.DrawImage(VerifyImage.charbmp, point);
			}
			solidBrush.Dispose();
			graphics.Dispose();
			graphics2.Dispose();
